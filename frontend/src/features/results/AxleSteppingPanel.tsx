...
export function AxleSteppingPanel({ jobId, videoId, trackId, onClose }: Props) {
  ...
  // Aks check'in suggest-frame endpoint'ini kullanıyoruz — aynı mantık
  ...
  return (
    <div className="space-y-3 rounded-lg border bg-muted/30 p-4">
      ...
      <p className="text-xs text-muted-foreground">
        Aracın <span className="font-semibold text-green-600">ön</span> (1. tıklama) ve{' '}
        <span className="font-semibold text-blue-600">arka</span> (2. tıklama) teker temas
        noktasını işaretleyin, dingil mesafesini girin. Arka teker eski ön tekerin konumuna
        geldiğinde araç tam 1 dingil mesafesi ilerlemiştir — H-tabanlı hızla bağımsız
        karşılaştırma üretir.
      </p>
      ...
      {frameN != null && (
        <>
          ...
          {/* Renk rehberi — CalibrationCanvas renkleriyle eşleşiyor */}
          ...
          <div className="flex flex-wrap items-end gap-3">
            ...
            <Button
              size="sm"
              disabled={points.length < 2 || stepMutation.isPending}
              onClick={() => stepMutation.mutate()}
            >
              {stepMutation.isPending ? <Loader2 className="animate-spin" /> : <Milestone />}
              Hesapla
            </Button>
      ...
      {stepMutation.isError && (
        <StatusBanner tone="error">{(stepMutation.error as Error).message}</StatusBanner>
      )}

      {stepMutation.isSuccess && data && (
            ...
            <>
              ...
              <div className="text-xs text-muted-foreground">
                ...
                {Math.abs(data.initial_distance_m - wheelbase) > 0.3 && (
                  <span className="text-amber-600">
                    {' '}— büyük fark, teker işaretlerini kontrol edin
                  </span>
                )})
              </div>

              <p className="text-[0.7rem] text-muted-foreground">
                Bu sonuç yalnızca çapraz doğrulama amaçlıdır. Confidence seviyesi hesabına
                dahil edilmez; PDF raporuna otomatik eklenmez.
              </p>
            </>
          )}
        </div>
